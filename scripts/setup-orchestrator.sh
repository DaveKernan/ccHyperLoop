...
jq -n \
  ...
  '{plan_path: $plan_path, base_branch: $base_branch, working_branch: $working_branch, clean_start_verified: true, max_concurrent: $max_concurrent, max_iterations: $max_iterations, max_retries: $max_retries, user_approved_dod: false, user_approved_concurrency: false, encouragement_enabled: false, token_optimization_enabled: false, test_command: "", start_command: "", has_ui: $has_ui}' \
